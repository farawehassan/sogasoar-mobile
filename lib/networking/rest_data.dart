...
import 'package:sogasoarventures/model/create_user.dart';
...
import 'package:sogasoarventures/model/product_history_details.dart';
...
class RestDataSource {
  ...
  static final BASE_URL = "https://sogasoar.herokuapp.com";

  static final LOGIN_URL = BASE_URL + "/authentication/login";
  static final SIGN_UP_URL = BASE_URL + "/authentication/signup";

  static final FETCH_USER_URL = BASE_URL + "/user/fetchAllUsers";
  static final DELETE_USER_URL = BASE_URL + "/user/deleteUser";
  ...
  /// A function that makes an admin fetch all users in the database
  /// [GET]
  Future<List<Workers>> fetchUsers() async {
    ...
    return _netUtil.get(FETCH_USER_URL, headers: header).then((dynamic res) {
      if(res["error"] == true){
        throw (res["message"]);
      }else{
        var rest = res["data"] as List;
        workers = rest.map<Workers>((json) => Workers.fromJson(json)).toList();
        return workers;
      }
    }).catchError((e){
      print(e);
      ...
      throw ("Error in fetching users, try again");
    });
  }

  /// A function that deletes a worker from the database using the [id]
  Future<dynamic> deleteWorker(String id) async {
    ...
    final DELETE_URL = DELETE_USER_URL + "/$id";
    return _netUtil.delete(DELETE_URL, headers: header).then((dynamic res) {
      ...
      throw ("Error in deleting user, try again");
    });
  }
